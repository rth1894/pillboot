...
EFI_STATUS assets_load(EFI_HANDLE ImageHandle, EFI_SYSTEM_TABLE *SystemTable, Assets *assets) {
    ...
    status = image_load(ImageHandle, SystemTable, L"\\EFI\\PillBoot\\assets\\right_hand.qoi", (Image *)&assets->rightHand);
    if (EFI_ERROR(status)) {
    ...
    status = image_load(ImageHandle, SystemTable, L"\\EFI\\PillBoot\\assets\\left_hand.qoi", (Image *)&assets->leftHand);
    if (EFI_ERROR(status)) {
    ...
    status = image_load(ImageHandle, SystemTable, L"\\EFI\\PillBoot\\assets\\red_pill.qoi", (Image *)&assets->redPill);
    if (EFI_ERROR(status)) {
    ...
    status = image_load(ImageHandle, SystemTable, L"\\EFI\\PillBoot\\assets\\blue_pill.qoi", (Image *)&assets->bluePill);
    if (EFI_ERROR(status)) {
